// heart-market.js - 心意集市（双钱包 · 自定义商品 · 内置心意记录 · 送礼走聊天）
(function() {
    'use strict';
        ...
        function scheduleNext() {
            ...
            window.__heartMarketTaGiftTimer = setTimeout(function() {
                try {
                    ...
                    var affordable = allItems.filter(function(it) { return it.price <= w.partnerBalance; });
                    if (affordable.length === 0) { scheduleNext(); return; }
                    var item = affordable[Math.floor(Math.random() * affordable.length)];

                    w.partnerBalance -= item.price;
                    _setWallet(w);

                    var words = _genGiftWords();
                    ...
                    if (typeof showNotification === 'function') {
                        showNotification('🎁 ' + member.name + ' 送了你一份礼物', 'info', 3500);
                    }
    ...
    window.openHeartMarket = function() {
        ...
        header.innerHTML = '<button id="hm-back" style="background:none;border:none;font-size:16px;color:var(--text-secondary);cursor:pointer;padding:4px 8px;">←</button>' +
            ...
            '<button id="hm-records-btn" style="background:none;border:none;font-size:13px;color:var(--accent-color);cursor:pointer;padding:4px 6px;font-weight:600;">📦 记录</button>' +
        ...
        function showRecordsDialog() {
            var oldDlg = document.getElementById('hm-records-dialog');
            if (oldDlg) oldDlg.remove();

            var currentRecTab = 'received';   // received / sent

            var dlg = document.createElement('div');
            dlg.id = 'hm-records-dialog';
            ...
            var innerHtml = '<div style="background:var(--primary-bg);border-radius:20px;width:min(420px,92vw);max-height:85vh;display:flex;flex-direction:column;overflow:hidden;border:1px solid var(--border-color);">' +
                '<div style="display:flex;justify-content:space-between;align-items:center;padding:14px 16px;border-bottom:1px solid var(--border-color);flex-shrink:0;">' +
                    '<span style="font-size:16px;font-weight:700;color:var(--text-primary);">📦 心意记录</span>' +
                    '<button id="hm-records-close" style="background:none;border:none;font-size:20px;cursor:pointer;color:var(--text-secondary);">✕</button>' +
                '</div>' +
                '<div style="display:flex;border-bottom:1px solid var(--border-color);flex-shrink:0;padding:0 16px;">' +
                    '<button class="hm-rec-tab active" data-tab="received" style="flex:1;padding:12px 4px 10px;border:none;background:transparent;font-weight:600;color:var(--text-primary);cursor:pointer;font-family:var(--font-family);font-size:14px;border-bottom:2px solid var(--accent-color);">📥 收到的</button>' +
                    '<button class="hm-rec-tab" data-tab="sent" style="flex:1;padding:12px 4px 10px;border:none;background:transparent;font-weight:400;color:var(--text-secondary);cursor:pointer;font-family:var(--font-family);font-size:14px;border-bottom:2px solid transparent;">📤 送出的</button>' +
                '</div>' +
                '<div id="hm-records-content" style="flex:1;overflow-y:auto;padding:14px 16px 20px;background:var(--secondary-bg);"></div>' +
                '</div>';
            dlg.innerHTML = innerHtml;
            document.body.appendChild(dlg);

            dlg.querySelector('#hm-records-close').onclick = function() { dlg.remove(); };
            dlg.onclick = function(e) { if (e.target === dlg) dlg.remove(); };

            function renderRecords() {
                var history = _getHistory();
                var list = history.filter(function(h) {
                    return h.action === currentRecTab;
                });
                list.sort(function(a, b) { return (b.ts || 0) - (a.ts || 0); });

                var contentEl = dlg.querySelector('#hm-records-content');

                if (list.length === 0) {
                    contentEl.innerHTML = '<div style="text-align:center;padding:60px 20px;color:var(--text-secondary);">' +
                        '<div style="font-size:48px;margin-bottom:12px;opacity:0.5;">🎁</div>' +
                        '<div style="font-size:14px;">' + (currentRecTab === 'received' ? '还没有收到过礼物~' : '还没有送出过礼物~') + '</div>' +
                        '</div>';
                    return;
                }
                ...
                for (var i = 0; i < list.length; i++) {
                    var h = list[i];
                    var otherName = (currentRecTab === 'received') ? (h.from || '群成员') : (h.to || '群成员');
                    var visualHtml = h.image
                        ? '<img src="' + _esc(h.image) + '" style="width:72px;height:72px;object-fit:cover;border-radius:14px;margin-bottom:6px;">'
                        : '<div style="font-size:52px;line-height:1;margin-bottom:6px;">' + _esc(h.emoji || '🎁') + '</div>';
                    ...
                    html += '<div style="background:var(--primary-bg);border-radius:16px;padding:14px 16px;margin-bottom:12px;border:1px solid var(--border-color);box-shadow:0 1px 4px rgba(0,0,0,0.03);">' +
                        '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">' +
                            ...
                            '</div>' +
                            '<div style="font-size:11px;color:var(--text-secondary);flex-shrink:0;">' + (h.ts ? _formatTime(new Date(h.ts).toISOString()) : '') + '</div>' +
                        '</div>' +
                        '<div style="display:flex;flex-direction:column;align-items:center;padding:12px 0;background:var(--secondary-bg);border-radius:12px;margin-bottom:10px;">' +
                            visualHtml +
                            '<div style="font-size:14px;font-weight:600;color:var(--text-primary);">' + _esc(h.name || '礼物') + '</div>' +
                        '</div>' +
                        (h.note ?
                            '<div style="padding:8px 12px;background:rgba(var(--accent-color-rgb),0.06);border-left:3px solid var(--accent-color);border-radius:6px;">' +
                                '<div style="font-size:11px;color:var(--accent-color);margin-bottom:4px;font-weight:600;">📝 ' + (currentRecTab === 'received' ? 'TA 的备注' : '我的备注') + '</div>' +
            ...
            dlg.querySelectorAll('.hm-rec-tab').forEach(function(btn) {
                btn.onclick = function() {
                    dlg.querySelectorAll('.hm-rec-tab').forEach(function(b) {
                    ...
                    currentRecTab = this.dataset.tab;
                    renderRecords();
                };
            });
        ...
        document.getElementById('hm-records-btn').onclick = showRecordsDialog;
    ...
    console.log('[心意集市] 模块已加载（内置心意记录 · 送礼走聊天）');
})();
