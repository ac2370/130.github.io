/* 核心应用逻辑：数据加载保存、消息渲染、会话管理等
   ...
   【修复版 v2 - 2026-09-18】修复多角色异步回复串框/丢失：
   1. addMessage：非当前角色的消息一律写入该角色专属存储池（不再静默丢弃）
   2. 新增 _currentContactId() 作为"当前角色"唯一权威来源，所有角色锁判断改用它
   3. simulateReply 先 await 读取"发起角色"的回复库/称呼/延迟/组合设置，内容完全隔离
   4. _triggerDelayedReply 改为按角色独立的待回复定时器池，切换后旧角色回复不被打断
   5. switchActiveContact / initializeSession 同步 window.SESSION_ID
*/
...
// 【修复】当前角色唯一权威来源 + 每角色待回复定时器池 + 按角色串行写入队列

// 切换角色时 switchActiveContact 会同步 currentContactId / SESSION_ID / window.SESSION_ID

// ============================================================

function _currentContactId() {

    return window.currentContactId || window.SESSION_ID || SESSION_ID;

}

window._currentContactId = _currentContactId;

// 每个角色独立的待回复定时器（防止给 B 发消息时把 A 的待回复定时器清掉）
window._pendingReplyTimers = window._pendingReplyTimers || {};

// 按角色串行化的后台写入队列：避免多条后台回复并发读写同一存储键造成互相覆盖
window._roleWriteQueues = window._roleWriteQueues || {};

function _pushToRoleStorage(contactId, message) {

    if (!contactId) return Promise.resolve();

    const prev = window._roleWriteQueues[contactId] || Promise.resolve();

    const next = prev.then(async () => {

        const key = `${APP_PREFIX}${contactId}_chatMessages`;
        ...
        console.log('[addMessage] 后台写入到', contactId, '消息数:', existing.length);
    ...
    // 吞掉失败，避免阻塞后续写入
    window._roleWriteQueues[contactId] = next.catch(() => {});
...
// 【核心】addMessage —— 带角色锁，防串框（修复版）

// 路由规则：
//   - contactId === 当前角色 → 正常渲染到当前聊天框
//   - contactId !== 当前角色 → 一律写入该角色专属存储池（绝不丢弃、绝不渲染进当前框），
//     silent 仅控制是否弹通知/提示音
// ============================================================
...
const addMessage = (message, opts) => {
    ...
    if (!message.contactId) {
        ...
        message.contactId = _currentContactId();

    }

    const isCurrentContact = (message.contactId === _currentContactId());

    // 【修复】后台写入：无论 silent 与否都写入该角色自己的存储池，保证异步回复不丢失
    if (!isCurrentContact) {

        const writePromise = _pushToRoleStorage(message.contactId, message);
...
// 【核心】_triggerDelayedReply —— 绑定发起时的 SESSION_ID（修复版）

// 每个角色使用独立的待回复定时器：切换角色后，旧角色的回复继续后台进行，
// 不会被新角色发消息时的本函数清掉
...
window._triggerDelayedReply = function(isUserMessage) {
    ...
    // 【关键】锁定发起时的角色 ID
    const originContactId = _currentContactId();
    ...
    if (isUserMessage) {
        ...
        setTimeout(() => {
            ...
            if (_currentContactId() !== originContactId) return; // 已切换，跳过
    ...
    // 【修复】按角色独立的待回复定时器池
    if (!window._pendingReplyTimers) window._pendingReplyTimers = {};

    if (window._pendingReplyTimers[originContactId]) {

        clearTimeout(window._pendingReplyTimers[originContactId]);

        window._pendingReplyTimers[originContactId] = null;
    ...
    window._pendingReplyTimers[originContactId] = setTimeout(() => {
    ...
    // 兼容旧代码对 window._pendingReplyTimer 的引用（指向当前角色的定时器）
    window._pendingReplyTimer = window._pendingReplyTimers[originContactId];
...
// 【核心】simulateReply —— 全流程角色锁（修复版）

// 1) 先 await 读取"发起时所属角色"的回复库/称呼/延迟/组合设置（内容完全隔离）
// 2) 每条回复 addMessage 时由 addMessage 内部按 contactId 自动路由：
//    当前角色 → 正常渲染；其他角色 → 写入该角色专属存储池（不串框、不丢失）

// ============================================================

async function _loadOriginReplyData(contactId) {

    const isCurrent = (contactId === _currentContactId());

    if (isCurrent) {

        // 当前角色：直接用内存全局（loadData 已按角色加载）
        return {
            ...
            stickerLibrary: Array.isArray(stickerLibrary) ? stickerLibrary : [],

            customReplyGroups: Array.isArray(window.customReplyGroups) ? window.customReplyGroups : [],

            partnerName: settings.partnerName || '对方',

            replyDelayMin: settings.replyDelayMin,

            replyDelayMax: settings.replyDelayMax,

            combineReplyCards: settings.combineReplyCards,

            combineReplyMaxCards: settings.combineReplyMaxCards,

            emojiMixEnabled: settings.emojiMixEnabled

        };

    }

    // 非当前角色：从该角色专属存储读取（保存时 getStorageKey 按 SESSION_ID 分角色）
    try {

        const prefix = `${APP_PREFIX}${contactId}_`;

        const [cr, ce, sl, crg, cs] = await Promise.all([
            ...
            localforage.getItem(prefix + 'chatSettings')

        ]);

        return {

            customReplies: Array.isArray(cr) ? cr : [],

            customEmojis: Array.isArray(ce) ? ce : [],

            stickerLibrary: Array.isArray(sl) ? sl : [],

            customReplyGroups: Array.isArray(crg) ? crg : [],

            partnerName: (cs && cs.partnerName) || '对方',

            replyDelayMin: (cs && cs.replyDelayMin) != null ? cs.replyDelayMin : 3000,

            replyDelayMax: (cs && cs.replyDelayMax) != null ? cs.replyDelayMax : 7000,

            combineReplyCards: !!(cs && cs.combineReplyCards),

            combineReplyMaxCards: (cs && cs.combineReplyMaxCards) || 3,

            emojiMixEnabled: !cs || cs.emojiMixEnabled !== false

        };

    } catch (e) {

        console.warn('[simulateReply] 读取发起角色数据失败:', e);

        return {

            customReplies: [], customEmojis: [], stickerLibrary: [], customReplyGroups: [],

            partnerName: '对方', replyDelayMin: 3000, replyDelayMax: 7000,

            combineReplyCards: false, combineReplyMaxCards: 3, emojiMixEnabled: true
...
window.simulateReply = async function(originContactId) {
    ...
    // 【修复】读取"发起时所属角色"的回复库/称呼/延迟/组合设置（await 后才开始生成）
    // 不再依赖当前角色的内存全局，杜绝回复内容、称呼串到另一个角色
    const originData = await _loadOriginReplyData(originContactId);

    const poolCustomReplies = originData.customReplies;

    const poolCustomEmojis = originData.customEmojis;

    const poolStickerLibrary = originData.stickerLibrary;

    const poolCustomReplyGroups = originData.customReplyGroups;

    const capturedPartnerName = originData.partnerName || '对方';

    const originDelayMin = Number(originData.replyDelayMin) || 3000;

    const originDelayMax = Number(originData.replyDelayMax) || 7000;

    const originCombine = !!originData.combineReplyCards;

    const originCombineMax = Math.max(1, Math.min(5, parseInt(originData.combineReplyMaxCards, 10) || 3));

    const originEmojiMix = originData.emojiMixEnabled !== false;
    ...
    for (let i = 0; i < replyCount; i++) {
        ...
        delay += originDelayMin + Math.random() * (originDelayMax - originDelayMin);
        ...
        setTimeout(() => {
            ...
            try {
                ...
                // 【修复】不再依赖 entry 时捕获的 isSameContact 决定 silent；
                // addMessage 内部会按 contactId 自动路由：
                //   当前角色 → 渲染；其他角色 → 写入该角色专属存储池（切换回来能看到）
                addMessage({
...
// 【修复】同步 window.SESSION_ID，确保 addMessage / simulateReply 的角色锁
// 始终用"最新当前角色"判断，杜绝 A 的回复被渲染进 B 的聊天框
...
window.switchActiveContact = async function(nextRole, nextName) {
    ...
    // 【关键】不清除 _pendingReplyTimers / _pendingReplyTimer！
