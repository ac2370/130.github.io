...
(function() {
    ...
    function _getRandomGroupMember() {
        ...
        if (members.length === 0) return { name: '群成员', avatar: '' };
    ...
    function showCoverSettings() {
        ...
        inner.innerHTML = '<div style="display:flex;justify-content:space-between;margin-bottom:14px;">' +
            ...
            '<input id="cover-url-input" type="text" placeholder="https://..." style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:12px;background:var(--secondary-bg);color:var(--text-primary);font-size:13px;box-sizing:border-box;font-family:var(--font-family);">' +
            '<button id="cover-url-btn" style="width:100%;margin-top:8px;padding:10px;border:1.5px dashed var(--border-color);border-radius:12px;background:transparent;color:var(--text-secondary);cursor:pointer;font-size:13px;font-family:var(--font-family);">🔗 加载图片</button>' +
            '</div>' +
    ...
    function editMyInfo() {
        ...
        inner.innerHTML =
            ...
            '<div style="display:flex;flex-direction:column;align-items:center;gap:12px;margin-bottom:16px;">' +
                '<div style="width:64px;height:64px;border-radius:50%;overflow:hidden;border:2px solid var(--border-color);display:flex;align-items:center;justify-content:center;background:var(--secondary-bg);position:relative;">' +
                ...
                '</div>' +
                '<div style="width:100%;">' +
                    '<label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">头像URL</label>' +
                    '<input id="edit-my-avatar-url-input" type="text" placeholder="粘贴图片URL" value="' + _esc(myAvatar) + '" style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:10px;background:var(--secondary-bg);color:var(--text-primary);font-size:12px;box-sizing:border-box;font-family:var(--font-family);">' +
                '</div>' +
                '<div style="width:100%;">' +
                    '<label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">昵称</label>' +
                    '<input id="edit-my-name-input" type="text" value="' + _esc(myName) + '" maxlength="12" style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:10px;background:var(--secondary-bg);color:var(--text-primary);font-size:14px;box-sizing:border-box;font-family:var(--font-family);">' +
                '</div>' +
            '</div>' +
            '<div style="display:flex;gap:10px;">' +
        ...
        // 实时预览头像URL
        document.getElementById('edit-my-avatar-url-input').addEventListener('input', function() {
            ...
            if (preview) {
                if (url) {
                    if (preview.tagName === 'IMG') {
                        ...
                        preview.outerHTML = '<img id="edit-my-avatar-preview" src="' + _esc(url) + '" style="width:100%;height:100%;object-fit:cover;">';
                    }
                } else {
                    if (preview.tagName === 'IMG') {
                        preview.outerHTML = '<span id="edit-my-avatar-preview" style="font-size:28px;">👤</span>';
                    }
                }
            }
        });
    ...
    function editMember(name) {
        ...
        inner.innerHTML =
            ...
            '<div style="display:flex;flex-direction:column;align-items:center;gap:12px;margin-bottom:16px;">' +
                ...
                '<div style="width:100%;">' +
                    '<label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">头像URL</label>' +
                    '<input id="edit-member-avatar-url-input" type="text" placeholder="粘贴图片URL" value="' + _esc(member.avatar || '') + '" style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:10px;background:var(--secondary-bg);color:var(--text-primary);font-size:12px;box-sizing:border-box;font-family:var(--font-family);">' +
                '</div>' +
                '<div style="width:100%;">' +
                    '<label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">昵称</label>' +
                    '<input id="edit-member-name-input" type="text" value="' + _esc(member.name) + '" maxlength="12" style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:10px;background:var(--secondary-bg);color:var(--text-primary);font-size:14px;box-sizing:border-box;font-family:var(--font-family);">' +
                '</div>' +
            '</div>' +
            '<div style="display:flex;gap:10px;">' +
        ...
        // 实时预览头像URL
        document.getElementById('edit-member-avatar-url-input').addEventListener('input', function() {
            var url = this.value.trim();
            ...
            if (preview) {
                if (url) {
                    if (preview.tagName === 'IMG') {
                        ...
                        preview.outerHTML = '<img id="edit-member-avatar-preview" src="' + _esc(url) + '" style="width:100%;height:100%;object-fit:cover;">';
                    }
                } else {
                    if (preview.tagName === 'IMG') {
                        preview.outerHTML = '<span id="edit-member-avatar-preview" style="font-size:28px;">🌸</span>';
                    }
                }
            }
        });
    ...
    function addMember() {
        ...
        inner.innerHTML =
            ...
            '<div style="display:flex;flex-direction:column;align-items:center;gap:12px;margin-bottom:16px;">' +
                ...
                '<div style="width:100%;">' +
                    '<label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">头像URL</label>' +
                    '<input id="add-member-avatar-url-input" type="text" placeholder="粘贴图片URL（可选）" style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:10px;background:var(--secondary-bg);color:var(--text-primary);font-size:12px;box-sizing:border-box;font-family:var(--font-family);">' +
                '</div>' +
                '<div style="width:100%;">' +
                    '<label style="font-size:12px;color:var(--text-secondary);display:block;margin-bottom:4px;">成员名字</label>' +
                    '<input id="add-member-name-input" type="text" placeholder="输入名字" maxlength="12" style="width:100%;padding:10px;border:1px solid var(--border-color);border-radius:10px;background:var(--secondary-bg);color:var(--text-primary);font-size:14px;box-sizing:border-box;font-family:var(--font-family);">' +
                '</div>' +
            '</div>' +
            '<div style="display:flex;gap:10px;">' +
        ...
        // 实时预览头像URL
        document.getElementById('add-member-avatar-url-input').addEventListener('input', function() {
            var url = this.value.trim();
            var preview = document.getElementById('add-member-avatar-preview');
            if (preview && url) {
                preview.outerHTML = '<img id="add-member-avatar-preview" src="' + _esc(url) + '" style="width:100%;height:100%;object-fit:cover;">';
            } else if (preview && !url) {
                preview.outerHTML = '<span id="add-member-avatar-preview" style="font-size:28px;color:var(--text-secondary);">+</span>';
            }
        });
